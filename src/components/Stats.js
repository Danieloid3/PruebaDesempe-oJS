...
export async function Stats() {
    ...
    const totalTasks = products.length;
    const completedTasks = products.filter(p => (p.status || 'pending') === 'completed').length;
    const pendingTasks = products.filter(p => (p.status || 'pending') === 'pending').length;
    const highPriorityPending = products.filter(
        p =>
